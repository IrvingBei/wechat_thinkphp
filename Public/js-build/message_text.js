...
function initFindSku(MOD){
	...
	var BTN_CHOOSE_SKU = $('#TEXT-CHOOSE-BTN'),
		WINDOW_CHOOSE_SKU = $('#TEXT-CHOOSE-WINDOW'),
            ...
            success: function(JSON){
                ...
                if(STATUS == 1){
                    ...
                    for(var i = 0, j = DATA.length; i < j; i++){
                        LIST_CHOOSE_SKU.append( SKU_LIST_TPL(i,DATA[i].id, DATA[i].content) );
                    }
                }else{
                    EMPTY.show();
                }
    ...
    var SKU_LIST_TPL = function(i, id, content){
        return '<tr>' +
                    '<td class="text-center">'+ id +'</td>' +
                    '<td class="text-center">'+ content +'</td>' +
                    '<td class="text-center">' +
                        '<button type="button" class="btn btn-xs btn-danger" data-text-id="'+ id +'" data-content="'+ content +'">'+ BUTTON_TEXT +'</button>' +
                    '</td>' +
                    '</tr>';
    }
